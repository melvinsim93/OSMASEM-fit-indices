...
##Distribution of Ns from the brief survey of studies that have used meta-analytic SEM
...
########################################################################Script to run the actual simulation##################################################################################
##Variables for the simulation - set variables/ conditions you want in the simulation here. Once done select the whole script and run it. 
##Once complete the script will produce three files with the estimates from the meta-analyses, fit indices and the sample sizes used for each condition
simulation.design<-expand.grid(
  modeltype<-c("cfa"), #Two options only: "cfa" and "path"
  modelsize<-c("small","large"), #Two options only: "small", "large"
  studysize<-c(20, 50, 100, 200), #Any number of studies, use whole numbers only
  modelmisspecification<-c(0, 20, 40, 60, 80), #Use only tens e.g. 10, 20, 30 etc.
  missingstud<-c(0), #Refers to the proportion of studies that will have missing variables, not proportion of studies that will be missing. Use only decimals between 0 to 1 to one decimal place
  missingvar<-c(0), #Refers to how many variables will be missing in each study, used in conjunction with missing studies to specify proportion of studies that will be affected. Use only decimals between 0 to 1 to one decimal place
  hetero<-c(0.1), #Refers to variance of correlations. Use only decimals from 0 to 1 with one decimal place.
  stringsAsFactors = F) #Do not touch this

simulate.no <- 1 #Number of simulations to run per condition. Immensensely CPU heavy and time consuming, do not run more than 10.

set.seed(14051996)

##To iterate over the conditions of the study design
...
for(j in 1:nrow(test)){
  #simulations experimental conditions
  sim.model.type <- test[j,1]
  sim.model.size <- test[j,2]
  sim.k <- test[j,3]
  sim.mod.misspec <- test[j,4]
  #sim.mod.misspec <- 90
  sim.miss.stud <-test[j,5]
  sim.miss.var <-test[j,6]
  sim.hetero <-test[j,7]
  #sim.hetero <- 0.1
  
  settings<-c(sim.model.type, sim.model.size, sim.k, sim.mod.misspec, sim.miss.stud, sim.miss.var, sim.hetero)
  
  cat("Beginning: ", "simulation ", j," out of ", nrow(test), ".\n", sep="")
  print(paste("Running with these conditions: ", "model.type: ", sim.model.type,
              "model.size: ", sim.model.size,
              "K: ", sim.k,
              "mod misspsec: ", sim.mod.misspec,
              "miss stud: ", sim.miss.stud,
              "miss var: ", sim.miss.var,
              "hetero: ", sim.hetero))
  
  hold<-simulate.it(model.type = sim.model.type, model.size = sim.model.size, 
                    k = sim.k, model.misspecification = sim.mod.misspec,
                    missingstud = sim.miss.stud, 
                    missingvar =  sim.miss.var, hetero = sim.hetero, simulate.no = simulate.no, cond.no = j)
  
  estimates[[j]] <- hold[[1]]
  fit.ind[[j]] <- hold[[2]]
  alltheN[[j]] <- hold[[3]]
  
  cat("Completed: ", "simulation ", j," out of ", nrow(test), ".\n", sep="")
}

write.csv(do.call("rbind", estimates), file = "estimates.csv")
write.csv(do.call("rbind", fit.ind), file = "fitind.csv")
write.csv(do.call("rbind.fill", alltheN), file = "alltheN.csv")
